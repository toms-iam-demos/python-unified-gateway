...
def test_forward_method_body_queries_and_secret_isolation(monkeypatch):
    ...
    c=TestClient(app())
    for method,path,body in [('GET','templates',b''),('POST','envelopes',b'{"status":"created"}'),('DELETE','brands/abc',b'')]:
        r=c.request(method,f'/docusign/esignature/v2.1/accounts/{ACCOUNT}/{path}?x=1&x=2',content=body,headers={'Authorization':'Basic operator-secret','content-type':'application/json'})
...
def test_encoded_path_escape_cannot_reach_provider(monkeypatch):
    ...
    for value in ['%252e%252e','%253fquery','%255cescape']:
        assert c.get(f'/docusign/esignature/v2.1/accounts/{ACCOUNT}/templates/{value}').status_code==422
